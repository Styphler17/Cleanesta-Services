...
<div class="mb-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    ...
    <button onclick="document.getElementById('addModal').classList.remove('hidden')" class="px-4 py-2 bg-primary text-white rounded hover:bg-secondary transition w-full sm:w-auto">+ Add Service</button>
</div>
<!-- Desktop Table -->
        ...
        <tbody>
            ...
            <tr class="border-b hover:bg-gray-50">
                ...
                <td class="px-2 sm:px-4 py-3 space-x-2">
                    <button onclick="openEditModal(<?php echo $service['id']; ?>)" class="px-3 py-1 bg-secondary text-white rounded hover:bg-primary transition">Edit</button>
...
<div class="sm:hidden space-y-4">
    ...
    <div class="bg-white rounded-lg shadow p-4 flex flex-col gap-2">
        <div class="flex justify-between items-center">
            <span class="font-bold text-primary"><?php echo $service['name']; ?></span>
        ...
        <div class="flex gap-2 mt-2">
            <button onclick="openEditModal(<?php echo $service['id']; ?>)" class="flex-1 px-3 py-2 bg-secondary text-white rounded">Edit</button>
...
<div id="addModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
    <div class="bg-white p-4 sm:p-8 rounded-xl shadow-lg w-full max-w-lg relative mx-2">
        <button onclick="document.getElementById('addModal').classList.add('hidden')" class="absolute top-2 right-2 text-gray-400 hover:text-primary text-2xl">&times;</button>
        <h2 class="text-xl font-bold mb-4 text-primary">Add Service</h2>
        <form method="post">
            <div class="mb-4">
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-2">
                ...
                <button type="submit" name="add_service" class="px-4 py-2 bg-primary text-white rounded hover:bg-secondary transition w-full sm:w-auto">Add</button>
            </div>
        </form>
    </div>
</div>
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 hidden">
    <div class="bg-white p-4 sm:p-8 rounded-xl shadow-lg w-full max-w-lg relative mx-2">
        <button onclick="document.getElementById('editModal').classList.add('hidden')" class="absolute top-2 right-2 text-gray-400 hover:text-primary text-2xl">&times;</button>
        <h2 class="text-xl font-bold mb-4 text-primary">Edit Service</h2>
        <form method="post" id="editForm">
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-2">
                ...
                <button type="submit" name="edit_service" class="px-4 py-2 bg-primary text-white rounded hover:bg-secondary transition w-full sm:w-auto">Update</button>
            </div>
        </form>
    </div>
</div>
